...
.getLevels.keeporder=function(x){
  ...
  res=NULL;
  if(length(x)>0){
    temp=data.frame(xv=x,index=c(1:length(x)));
    ...
    Tempdata=Tempdata[order(Tempdata[,"index"]),,drop=F];
    res=as.character(Tempdata[,"xv",drop=T]);
    #print(Tempdata);
  }
  return(res);
}
...
.getLevels.keeporder.data.frame=function(x){
  ...
  if(nrow(x)>0 && ncol(x)){
    ...
    if(nrow(x)>1)for(RowIndex in 2:nrow(x)){
      NewItem=T;
      crntrow=as.vector(t(x[RowIndex,,drop=F]));
      for(Index in 1:nrow(res)){
        crntItem=as.vector(t(res[Index,,drop=F]));
...
.getsubset.byvar=function(byvarnames, byvarvalues,mydata,truefalse=T){
  res=data.frame();
  if(dim(mydata)[1]*dim(mydata)[2]>0){
    ...
    if(length(byvarnames)>0){
      ...
      for(Index in 1:length(byvarnames)){
        rowindicators=res[,byvarnames[Index],drop=T]==byvarvalues[Index];
        if(!truefalse[Index]){
          rowindicators = (!(rowindicators));
        }
        res = res[rowindicators,,drop=F];
      }
    }
  }
  return(res);
...
.getRowAsVector=function(x,RowIndex){
  return(as.vector(t(x[RowIndex,,drop=F])));
}

.deleteLastInterval.onecorhot = function(yearcol,intervalcol,mydata){
  res=NULL;
  if(dim(mydata)[1]*dim(mydata)[2]>0){
    YearRange=as.numeric(.getLevels.keeporder(mydata[,yearcol,drop=T]));
    ...
    for(YearValue in YearRange){
      ...
      #get interval values
      IntervalRange=as.numeric(.getLevels.keeporder(cntYearData[,intervalcol,drop=T]));
      ...
      res=rbind(res,.getsubset.byvar(byvarnames=intervalcol,byvarvalues=DelIntervalValue,cntYearData,truefalse=F));
    }
  }
  return(res);
}
...
#    formu: the forumula in the form ~nAlive+nDied+nLost+expSurv+Interval+year
#    dataSource: the data source
.joinpoint.old = function(formula, data, subset, numJPoints = 0, delLastIntvl=F,...) {
	is.seerstat = FALSE;
	...
	if(delLastIntvl){
	 dataMatrix=deleteLastInterval(data=dataMatrix,byvarnames=c(),yearcol=colnames(dataMatrix)[6],intervalcol=colnames(dataMatrix)[5]);
	}
	nAlive = dataMatrix[, 1];
	nDied = dataMatrix[, 2];
	nLost = dataMatrix[, 3];
	...
	plot.surv = function() {
		...
		plot(pred1$Year + 1975, pred1$pred_cum, type = "l", lwd = 3, col = "black", xlab = "Year of diagnosis", ylab = "relative survival", ylim = c(0, 1));
		pred2 = subset(pred1, Year %in% (cox_fit$jp));
		lines(pred2$Year + 1975, pred2$pred_cum, type = "p", pch = 20, cex = 1.8, col = "black");
		pred1 = subset(pred, Interval == 3);
		lines(pred1$Year + 1975, pred1$pred_cum, type = "l", lwd = 3, col = "darkgreen");
		pred2 = subset(pred1, Year %in% (cox_fit$jp));
		lines(pred2$Year + 1975, pred2$pred_cum, type = "p", pch = 20, cex = 1.8, col = "darkgreen");
		pred1 = subset(pred, Interval == 5);
		lines(pred1$Year + 1975, pred1$pred_cum, type = "l", lwd = 3, col = "darkorange");
		pred2 = subset(pred1, Year %in% (cox_fit$jp));
		lines(pred2$Year + 1975, pred2$pred_cum, type = "p", pch = 20, cex = 1.8, col = "darkorange");
	
		# Plot the observed values of the 1-, 3-, 5-year survival
		lifeDat2 = subset(dataMatrix, Interval == 1);
		lines(lifeDat2[, 6] + 1975, lifeDat2[, 7], type = "p", col = "black");
		lifeDat2 = subset(dataMatrix, Interval == 3);
		lines(lifeDat2[, 6] + 1975, lifeDat2[, 7], type = "p", col = "darkgreen");
		lifeDat2 = subset(dataMatrix, Interval == 5);
		lines(lifeDat2[, 6] + 1975, lifeDat2[, 7], type = "p", col = "darkorange");
  ...
  FitList=fitres$FitList;
	for(Index in 0:numJPoints){
	 if(BestNumJP == Index){
	   FitList[[Index+1]]=cox_fit;
	 }else{
    pred = FitList[[Index+1]]$Predict();
    FitList[[Index+1]]$predicted = merge(dataMatrix, pred, by.x = c(6, 5), by.y = c("Year", "Interval"));
	 }	
	}
  cox_fit$FitList=FitList;
	cox_fit$plot.surv = plot.surv;
	#cox_fit$dataMatrix=dataMatrix;
